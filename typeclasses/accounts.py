...
class Account(DefaultAccount):
    ...
    * Hook methods (when re-implementation, remember methods need to have self as first arg)
    ...
    def _send_to_connect_channel(self, message):
        ...
        """
        self._send_to_info_channel(message)

    pass

    def _send_to_info_channel(self, message):
        """
        Helper method for loading and sending to the comm channel
        dedicated to information messages.
        ...
        """
        _MUDINFO_CHANNEL = ChannelDB.objects.filter(db_key=settings.CHANNEL_MUDINFO["key"])[0]
        if settings.USE_TZ:
        ...
        if _MUDINFO_CHANNEL:
            ...
            _MUDINFO_CHANNEL.tempmsg(f"[{now}]: {message}")
        else:
